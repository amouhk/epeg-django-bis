...
if ($action == 'simple') {
    ...
    if ($handle->uploaded) {
        ...
        $handle->file_src_name_body = 'VIDEO_' . $str_id;
        $dir_dest = $dir_dest ."/Video/". $album;
        ...
        if ($handle->processed) {
            ...
            if ( !empty($mysqli) )
            {
                $sql = "INSERT INTO `galeries`(`id`, `name`, `type`, `album`, `filepath`) VALUES (NULL, '$handle->file_src_name_body', 'Video', '$album', '$db_file_path')";
