...
// set variables
$dir_dest = (isset($_GET['dir']) ? $_GET['dir'] : '/home/epegobel/public_html/ressources/programs');
...
if ($action == 'simple') {
    ...
    $handle = new upload($_FILES['inputPdf']);
    ...
    if ($handle->uploaded) {
        ...
        if ($handle->processed) {
            ...
            if ( !empty($mysqli) )
            {
              $sql = "UPDATE `programmes` SET `cheminPdf` = '$db_file_path' WHERE `programmes`.`id` = 1;";
