...
def home(request):
    ...
    photo_carousel_active = Gallery.objects.filter(album="noel2020")[0]
    photos_caroussel = Gallery.objects.filter(album="noel2020").reverse()
    return render(request, 'appv/index.html', locals())


def about(request):
    welcom_e = conf_settings.WELCOME_MSG
